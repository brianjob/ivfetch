...
class PGoApi:
    ...
    def heartbeat(self):
        ...
        if 'GET_INVENTORY' in res['responses']:
            ...
            dump = json.dumps(res['responses'], indent=2)
            self.log.info("%d pokemon in box", get_inventory_data(res, self.pokemon_names).count("\n"))
        ...
        for pokemons in caught_pokemon.values():
            if len(pokemons) > MIN_SIMILAR_POKEMON:
                ...
                for pokemon in pokemons[MIN_SIMILAR_POKEMON:]:
                    if 'cp' in pokemon and pokemonIVPercentage(pokemon) < self.MIN_KEEP_IV and pokemon['cp'] < self.KEEP_CP_OVER and pokemon['pokemon_id'] != 16 and pokemon['pokemon_id'] != 13:
